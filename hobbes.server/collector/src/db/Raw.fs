...
module Raw =
    ...
    let private db = 
        Database.Database("raw", CacheRecord.Parse, Log.loggerInstance)
          .AddView("table")
          .AddView "WorkItemRevisions"

    let keys (source : DataSource) = 
        let startKey = 
    ...
    let setSyncState state message revision source = 
        ...
        let doc = """{"msg": "This is not implemented yet"}"""                            
    ...
    let tryLatestId (source : DataSource) =
