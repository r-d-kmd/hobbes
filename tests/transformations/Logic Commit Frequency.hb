...
create column "date" (format date "committer.date" date)
only ((format date "date" year) = 2019 && (format date "date" month) > 6) || ((format date "date" year) >= 2020)
create column "tick" 1
slice columns "date" tick
group by date -> sum
rename column tick "count"
sort by column date
create column Frequency (moving mean 30 ["count"])
create column "Frequency long" (moving mean 90 ["count"])
slice columns Frequency "Frequency long"
only !(Frequency = missing)


!### The method
The Data is collected from the Azure DEvOps REST api. becuase there are large fluctuations in the numbers the numbers are calculated pr day and as a running average of 30 days
!#