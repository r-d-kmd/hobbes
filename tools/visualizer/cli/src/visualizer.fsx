...
(*let pie : seq<(int*System.IConvertible)> -> XPlot.Plotly.Chart = 
    Seq.head
    >> XPlot.Plotly.Chart.Pie

let buble (data : seq<(int*System.IConvertible)>) = 
    (data |> Seq.head)
...
let filename = __SOURCE_DIRECTORY__ + "/chart.html"
System.IO.File.WriteAllText(filename,renderedChart.GetHtml())
